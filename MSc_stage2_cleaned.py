...
def initialise(h):
    # initialise stage 1 to access global variables
    stg1.initialise()
...
def main():
    ...
    with gp.Env(params=options) as env:
        ...
        L_BW = m.addMVar((cf.NUMBER_OF_SERVERS, cf.STEP), lb=0, ub=mf.L_max, name="L_BW")  # Data centre load
        ...
        for t in range(cf.STEP):
            ...
            # if heat_demand_5min_forecast[t] >= 0:
            #     m.addConstr(H_sub[t] == heat_demand_5min_forecast[t]*1e3 - H_1[t], name=f"heat_balance_{t}")
            # else:
            #     m.addConstr(H_sub[t] == 0, name=f"heat_balance_{t}")
